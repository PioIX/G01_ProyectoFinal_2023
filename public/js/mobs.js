...
async function getRandomInt() {
    mobPalabras = await fetchMobPalabras()
    console.log(mobPalabras)
    let randomNumber = Math.floor(Math.random() * mobPalabras.length)
    cargarPalabras(randomNumber);


    mobImagenes = await fetchMobImagenes()
    let relatedMobImage = mobImagenes[randomNumber].imagen
    var img = document.createElement('img'); 
    img.classList.add('relatedMobImage')
    img.src = `${relatedMobImage}`; 
...
function cargarPalabras(random) {
    word = mobPalabras[random].nombre_mob
    console.log(word)
